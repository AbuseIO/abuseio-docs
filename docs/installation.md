...
The .env file contains your base configuration and must be set correctly because you will be setting the application's configuration. update the file /opt/abuseio/.env and update the SQL username, password and database you have created in the previous step. the other content of .env is here for demo purposes:
...
```
### About keys in .env
WARNING: the APP_KEY and APP_ID should have a random value that was auto generated by composer. If these values are incorrect you will get weird encryption errors and error 500 while loading the GUI. If for any reason
the keys in these fields are wrong, you can fix it by setting the values to:
