...
```
### Updating keys [first time only]
If this is your first (clean) install (not updating), then the KEY/ID are set to DEFAULT. You will need to generate a pair:
...
if asked to run this in production, choose yes. After check the .env if DEFAULT part has changed into a random value.
